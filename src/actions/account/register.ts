...
export default class RegisterAction extends LoginAction implements BaseActionInterface {
  ...
  public async start() {
    try {
      ...
      if (!verifyStatus) throw new Error(`verify code(${input.uname}) is invalid.`)
  ...
  private async fillRegisterForm() {
    ...
    if (this.input.registerByMail) emailOrPhoneInput = this.input.mail
  ...
  private async fetchAndSubmitVerifyCodeFromFile(page: Page) {
  ...
  private async isVerifyCodeValid() {
    ...
    else if (this.page.$$) hasErrorArr = await this.page.$$('text=something went wrong')
    else {
      await this.bot.delay('high')
      if (this.page.$$)
        hasErrorArr = await this.page.$$("text=That code isn't valid. You can request a new one.")
    }
    if (hasErrorArr && hasErrorArr.length) return false
    return true
  }
    ...
    return new Promise(async function (resolve, reject) {
      ...
      var timeFlag: NodeJS.Timer = setInterval(async () => {
        if (count > 100) {
          ...
          reject('')
          throw new Error(
            'Timeout for set verify code, Please try again and set verify code in storage/verify.txt'
          )
        }
        if (!(await fs.existsSync(path))) await fs.writeFileSync(path, '')
  ...
  private async _fetchCodefromWeb(page: Page): Promise<any> {
    return new Promise(async function (resolve, reject) {
      var count = 0
  ...
  private async getEmail(page: Page) {
