...
export default class LsInstagramBot {
  ...
  private _mergeOptions(options: optionsInterface): optionsInterface {
    if (options.isDevelopment) return { ...this._defaultDevOptions, ...options }
    else return { ...this._defaultPrdOptions, ...options }
  }
  ...
  private async _createBrowserBot() {
    ...
    let browserContextName =
      this.options.storagePath + this.browserPersistentContextPrefix + '-' + this.options.botName
    this.browser = await browserObj[this.options.browser!].launchPersistentContext(
      browserContextName,
      this.options
    )
